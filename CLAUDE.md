...
- Claude helpers: subagents in `.claude/agents/` (engine-pixi-svelte, book-of-game, launcher-studio, atlas-python-tools, infra-railway, code-reviewer, docs-keeper), skill `/deploy`. Per-game-template agents (e.g. `book-of-game`) own a specific slot type and build on `engine-pixi-svelte`.

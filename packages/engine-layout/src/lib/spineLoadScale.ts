 ...
 * size passes {@link EDITOR_SPINE_LOAD_SCALE} to `<SpineProvider loadScaleBase>` — which
 ...
 * things (the win-overlay WYSIWYG bug). `apps/launcher-api/src/lib/spineScale.ts` re-exports
...
/**
 * COMPONENT / placed editor-art spines (a spin button, a free-spin frame, the big-win rig…).
 ...
 * convention" that, for a natural-sized spine (no width/height, e.g. a button whose atlas

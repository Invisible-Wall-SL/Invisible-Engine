...
def test_clearing_the_pick_clears_the_field() -> None:
    r = _saved(_card(variant=""), region={"name": "H1", "variant": "00009"})
...
def test_deleting_the_picked_file_drops_the_pin() -> None:
    ...
    u.variant_files = lambda name: sorted(batch.glob(f"{name}_*.png"))  # noqa: E731
    mp, _ = _manifest({"name": "H1", "variant": "00003"})
...
def _after_render(region: dict, pile: list[int | None],
    ...
    """Run _drop_superseded_picks the way run_render's _post does: snapshot
    the newest id, let the pile grow (or not), then re-read the pin."""
    batch = _pile(pile)
    u.variant_files = lambda name: sorted(batch.glob(f"{name}_*.png"))  # noqa: E731
    before = {region["name"]: u.newest_variant_id(region["name"])}
    for i, s in enumerate(rendered, start=len(pile) + 1):
        _variant_png(batch / f"{region['name']}_{i:05d}_.png", s)
    m = {"regions": [dict(region)]}
    u._drop_superseded_picks(m, before)
    return m["regions"][0]


def test_a_fresh_render_supersedes_an_unlocked_pick() -> None:
    r = _after_render({"name": "H1", "variant": "00002", "lock": False},
                      [11, 22], [99])
...
def test_a_fresh_render_leaves_a_locked_pick_alone() -> None:
    r = _after_render({"name": "H1", "variant": "00002", "lock": True},
                      [11, 22], [99])
...
def test_a_render_of_another_slot_keeps_this_ones_pick() -> None:
    batch = _pile([11, 22])
    u.variant_files = lambda name: sorted(batch.glob(f"{name}_*.png"))  # noqa: E731
    m = {"regions": [{"name": "H1", "variant": "00002", "lock": False}]}
    u._drop_superseded_picks(m, {"L1": ""})     # only L1 was rendered
    check("an untouched slot is not in the render's snapshot",
...
def test_the_card_shows_the_picked_file() -> None:
    batch = _pile([11, 22, 33])
    u.variant_files = lambda name: sorted(batch.glob(f"{name}_*.png"))  # noqa: E731
    thumb, full, cap, us = u.output_view("H1", {"name": "H1", "variant": "00002"})
...
def test_with_no_pick_the_card_shows_the_newest() -> None:
    batch = _pile([11, 22, 33])
    u.variant_files = lambda name: sorted(batch.glob(f"{name}_*.png"))  # noqa: E731
    thumb, _full, _cap, us = u.output_view("H1", {"name": "H1"})

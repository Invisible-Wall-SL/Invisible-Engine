...
def _placement_mode(r: dict, keep_full: bool = False) -> dict:
    ...
    order matters and is not re-derivable by eye:
      ...
      1. otherwise an EXPLICIT `fit_mode == "contain"` short-circuits to the
         ...
         packer.compose) — or, for an FX layer whose base compose can resolve,
         `_fx_parity_tile`, the same replay taken about the BASE's ink. Same
         branch either way, which is why this reports one mode: what changes is
         whose bbox is centred, and only for an FX layer, and only visibly for
         an offset drop shadow;
...
def _parity_of(region: dict, page: Image.Image,
    ...
    or, for an FX layer that compose places by its base's numbers rather than
    its own ink (`batch_atlas.fx_registration` says which), through the REAL
    `batch_atlas._fx_parity_tile` — predicting an offset drop shadow with the
    ...
    try:
        ...
        fx = batch_atlas.fx_registration(img, region, by_name or {}, BATCH_DIR)
        tile = (batch_atlas._fx_parity_tile(img, tw, th, rotated, fx)
                if fx is not None and not fx["replay"]

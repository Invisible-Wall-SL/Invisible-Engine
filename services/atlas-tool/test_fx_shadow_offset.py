...
THE FIX, in two halves, because the offset has to survive BOTH steps:
  ...
  * `batch_atlas.layer_registration` + `_layer_parity_tile` place an FX layer by its
...
def _compose_pair(td: Path, base_img: Image.Image, fx_img: Image.Image,
    ...
    resolved the same way (`layer_registration` off the bound art).
    ...
    try:
        ...
        reg = batch_atlas.layer_registration(fx_img, fx, by_name, td)
...
def test_the_canvas_grows_symmetrically_by_twice_the_offset() -> None:
    ...
    space (`layer_registration` maps the base's bbox straight across the margin)."""
    src = _glyph()
...
def test_a_rotated_cell_carries_the_drop_round_with_it() -> None:
    ...
    with tempfile.TemporaryDirectory() as t:
        ...
        for ox, oy in ((0, 8), (6, 0), (-9, -5)):
            ...
            try:
                b = batch_atlas.fit_to_region(src.copy(), base)
                reg = batch_atlas.layer_registration(sh, fx, {"T": base,
                                                           "T_shadow": fx}, td)
...
def test_a_region_that_is_not_an_fx_layer_is_never_registered() -> None:
    """The gate. `layer_registration` answers None for anything that is not a
    ...
    with tempfile.TemporaryDirectory() as t:
        ...
        check("a non-FX name registers nothing",
              batch_atlas.layer_registration(sh, plain, {"T": plain}, td), None)
        check("an FX name with no base registers nothing",
              batch_atlas.layer_registration(sh, orphan, {}, td), None)
...
def test_art_that_is_not_the_bases_canvas_is_refused() -> None:
    ...
    with tempfile.TemporaryDirectory() as t:
        ...
        got = {k: batch_atlas.layer_registration(
...
def test_the_alpha_paths_crop_box_is_unmoved_for_art_it_was_not_clipping(
    ...
    with tempfile.TemporaryDirectory() as t:
        ...
        for oy in (8, -8, 12):
            ...
            ob = batch_atlas.layer_registration(old, fx, by, td)["box"]
            nb = batch_atlas.layer_registration(new, fx, by, td)["box"]

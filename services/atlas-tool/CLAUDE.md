...
## How the port works
- **`cloud_paths.py`** — drop-in for the original `project_paths.py`. Same `resolve()` dict shape, but `input_dir/batch_dir/atlas_dir/manifest_dir` point at a **local staging dir** (`ATLAS_STAGING`, default `/data/atlas-tool`) that mirrors an R2 prefix `atlas_maker/cloud/<project>` 1:1. So the original `ui_server.py`/`batch_atlas.py` pathlib+PIL code runs unchanged. Adds `comfy_url`, `cf_headers`, `r2_project_prefix`.

...
| **`boot-splash.js`** — vanilla twin for the STATIC WebGL apps: auto-installs on parse (`<script data-tool="…" data-phrases="a\|b">` first in `<body>`), lifts on `IWBoot.done()`, falls open on `window.load`+400 ms and a 20 s cap | static | `apps/launcher-api/static/shared/boot-splash.js` — used by `static/{spine,rigger}/view.html` | canonical for the static line |

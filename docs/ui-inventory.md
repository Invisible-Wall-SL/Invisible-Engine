...
### 11. Colour picker (swatch + popover) + gradient editor (multi-stop ramp)
| Impl | Domain | File(s) | Status |
|---|---|---|---|
...
| **`GradientBar.svelte`** — Photoshop-style gradient editor: chequered ramp + a rail of house-shaped colour stops. Click the rail to add a stop (seeded with the colour the ramp already shows there), drag to move, drag it off the rail or press Delete to remove, arrow keys nudge; a diamond per gap is Photoshop's **midpoint** (where the 50/50 blend lands); per-stop **opacity**; `Delete` / `Reverse`. Props: `bind:stops` (`GradientStop[]`), `startLabel` / `endLabel` (what the ramp's ends mean in the caller's space), `disabled`. Uses `<ColorField>` for the stop colour. | A | `apps/launcher-api/src/lib/GradientBar.svelte` + `$lib/gradient.ts` (model + maths) | **canonical for domain A** — used by fonts (the Generate tab's gradient fill) |

→ **`$lib/gradient.ts` is the renderer-agnostic half** — keep the model there, not in the tool. A `GradientStop` is `{ at, color, alpha, mid }`; neither CSS nor Canvas has a midpoint, so `flattenStops()` resolves that power curve into plain offset/`rgba()` samples and **every** consumer paints the identical ramp: `cssGradient(stops)` for HTML/CSS, `applyStops(target, stops)` for anything with `addColorStop` (Canvas 2D's `CanvasGradient`, PIXI 8's `FillGradient` — the `ColorStopTarget` interface is structural for exactly that reason). `normalizeStops()` salvages arbitrary parsed JSON and `stopsFromPair()` upgrades a legacy two-colour doc, so a tool adopting the bar can migrate its saved docs without inventing a shape. To re-plug it anywhere in the launcher: store `GradientStop[]` in your doc, render `<GradientBar bind:stops={…} startLabel=… endLabel=… />`, and paint with the helper for your renderer. No domain-B (Python/static) twin exists yet — a vanilla one would follow the `color-field.js` pattern in this section.
...
| Impl | Domain | File(s) | Status |
|---|---|---|---|
| **`ConfirmDialog.svelte`** — native `<dialog>` + `showModal()`. Snippet `body`, plain-text `message` (newlines preserved), optional `requireText` typed guard, `input` (the `prompt()` shape), `hideCancel` (the `alert()` shape), `busy` (holds it open + un-dismissable while the action runs), `blocked` (a precondition typing can't defeat), `error` (a failed attempt rendered inside), `danger` fill. `onconfirm(typed)` hands the guard/input text back so the SERVER can re-validate it | A | `apps/launcher-api/src/lib/ConfirmDialog.svelte` | **canonical for domain A** — rendered directly by admin project delete + purge (the two that need `busy`/`error` across the action) |
| **`askConfirm()` / `askMessage()` / `askText()`** — promise-returning helpers over ONE `<DialogHost>` mounted in the `(app)` layout. FIFO queue, so an un-awaited ask can't be dropped. Reads like the `confirm()` it replaced: `if (!(await askConfirm({…}))) return;` | A | `apps/launcher-api/src/lib/dialogs.svelte.ts` + `DialogHost.svelte` | **canonical for domain A** — every tool page; 46 native pop-ups across 11 route files went through here (2026-09-18) |
| **`guardUnsavedWork()`** — the unsaved-work leave guard. `beforeNavigate` is synchronous and a dialog is not, so it CANCELS first and re-issues on confirm (`goto` for link/`goto`, `history.go(delta)` for Back/Forward) | A | `apps/launcher-api/src/lib/unsavedGuard.ts` | **canonical for domain A** — consumers: `components`, `sound` |
| `window.confirm()` / `alert()` / `prompt()` | A | — | **gone from `src/`**, pinned by `apps/launcher-api/nativeDialogs.fixture.ts` (`pnpm --filter launcher-api check:native-dialogs`). Only `static/rigger/cinematic.js` still has them: vanilla JS served as-is, it cannot import a Svelte component |
| Hand-rolled `.modal-backdrop` divs | A | `(app)/game-maker/+page.svelte` (×3), `(app)/config/+page.svelte`, `(app)/editor/+page.svelte` (`.lp-modal-backdrop`) | **legacy** — same target; these are rich panels, not questions, so they need `ConfirmDialog`'s `body` snippet rather than the promise helpers |

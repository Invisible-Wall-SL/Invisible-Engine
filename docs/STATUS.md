...
- **B23 — launcher CLIENT/PROJECT selector broken** ⏳ TODO, **high priority** (affects every project-scoped tool): nothing stays selected; selecting one reshuffles everything. Files: `(app)/+page.svelte`, `(app)/+page.server.ts` (`?/setProject`), `(app)/+layout.server.ts`, `lib/server/auth.ts`, `lib/server/projects.ts`, `lib/server/db/schema.ts`. Fix direction (rebuild): CLIENT as a pure local filter (never persists), persist only on explicit PROJECT change, seed once + don't clobber mid-edit, stop the load silently rewriting the active project, consider storing client explicitly alongside project.

...
5. **Engine consumption + per-game rollout.** Background bind nodes size via the shared cover in `LayoutNodeView` reading the SAME `backgroundCoverScale`/`backgroundFit`; games drop hardcoded scales, bump submodule, rebuild. Parity-gated per game (lines + Book of Borut).

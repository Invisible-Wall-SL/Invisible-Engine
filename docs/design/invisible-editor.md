...
- **Phase 1 (gaps/non-square) — editor-only, additive, parity-safe in-game.**
  ...
  - **Owner decision 2026-06-12: non-square keeps ENGINE symbol sizing** (no stretch/fit). So `cellWidth/cellHeight` drive the CELL box + per-axis PITCH (row/column spacing), NOT the symbol draw size — symbols stay engine-sized (`SYMBOL_SIZE·sizeRatios`), centred in the cell via padding. This simplifies Phase 2: no per-symbol sprite-aspect change, only positions/pitch.
...
- **Phase 2 (engine consumption).**
  ...
  - **Symbol sizing unchanged (owner decision):** art stays engine-sized (uniform `cellSize/SYMBOL_SIZE` zoom); gaps/non-square only move the lattice, so square art never distorts in a non-square cell.

...
- **Phase 2 (engine consumption) — ✅ LANDED for engine + `apps/lines` 2026-06-12 (parity browser-verified; Borut mirror pending).**
  ...
  - **Borut mirror — PENDING.** Engine (`engine-layout` + `utils-slots`) flows via a submodule bump; the `apps/lines` game-file edits (`stateGame` `getSymbolX`/`boardGeometry`/`boardLayout`, `ReelSymbol`, `utils`) must be mirrored into Book of Borut's own game files, then republish.

...
- **Phase A — free-spin screens in `apps/lines` (NOT done):** `apps/lines` still hardcodes `<FreeSpinIntro/>` etc.; bring it in line with Borut (which already mounts them via `<LayoutScene>`). Low-risk reference catch-up.

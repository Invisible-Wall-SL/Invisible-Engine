...
- **Phase 2 — engine consumption (parity-gated, per game).** Generalize `boardLayout()`/`getSymbolX-Y`/the reel loop/mask/frame + `createReelForSpinning` to read the `reelGrid` node from the doc when present, falling back byte-identically to `constants.ts` when absent/equal. Wire Borut first (bump submodule, republish), update its `defaultLayout.ts` to emit the node. Then the editor's grid edits reshape the real in-game board.
- **Phase 3 — later.** Win-line geometry (next primitive); per-layoutType grid overrides; flip the `reelGrid` template slot `required` and retire the coded board mount.

...
- **Phase 2 (engine consumption) — NEXT, parity-gated.** New geometry resolver (`{cellW, cellH, pitchX, pitchY, padX, padY, x, y}`); make `utils-slots` `symbolHeight` accept `number | (() => number)` so row pitch is reactive (backward-compatible — other games pass a number); route geometry into `getSymbolX`/symbol draw size/`BoardMask` in `apps/lines`; parity proof at no-node / gap 0 / square; mirror to Book of Borut (submodule bump). **Open decision:** non-square draws symbols stretched to `cellW×cellH` vs. fit-and-letterbox (square art in a non-square cell).
- **Phase 3 (spin-feel params).** Surface a `SPIN_OPTIONS` subset (reel spin speed/bounce/delay, motion blur) as an editable spin-tuning surface — separate from layout.

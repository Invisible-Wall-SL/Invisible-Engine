...
| `GITHUB_ACTIONS_TOKEN` | **Optional.** A GitHub token with **`actions: write`** on `GITHUB_ENGINE_REPO`. Powers **Rebuild image** on `/comfyui` (a `workflow_dispatch` of `atlas-comfy-pod.yml`) and reading that workflow's runs. Deliberately NOT the read-only `GITHUB_ENGINE_READ_TOKEN` — dispatching is a write and deserves its own blast radius. Unset → the button is replaced by a line saying so. |

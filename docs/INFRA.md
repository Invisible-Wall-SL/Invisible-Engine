...
> - **Anthropic** — `ANTHROPIC_ADMIN_API_KEY`, an **admin** key (`sk-ant-admin…`) from Console → Settings → Admin keys. A **different credential** from `ANTHROPIC_API_KEY` (which Localization spends with); a regular key gets 401 on the cost report. Note the API returns `amount` in **cents as a decimal string**, and exposes **no credit-balance endpoint** — remaining prepaid credit is derived from the admin-entered top-up ledger (`cost_top_ups`, migration `0015_loud_triton.sql`), never measured.

...
| Var | Service | Purpose |
|---|---|---|
| `COMFY_CATALOG_URL` | **atlas-tool** | **Optional — the RunPod side's model-list source.** Base URL of an always-on ComfyUI that mounts the same Network Volume the serverless workers use: the CPU volume pod the launcher's `COMFY_VOLUME_URL` points at, or any `/comfyui` fleet pod while it runs (`https://<pod-id>-8188.proxy.runpod.net`). ⚙ Settings reads it on **⟳ Refresh model lists** when *Run generation on* = RunPod and stores the lists in `_shared/comfy/catalog-pod.json`. Read-only (only `GET /system_stats` + `GET /object_info/<node>`), so it never starts, resumes or bills a pod; sends the same CF Access headers + `InvisibleAtlas/1.0` UA as every other ComfyUI call. Needed because the serverless endpoint is a job queue, not an HTTP server — there is nothing live to ask. It is NOT consulted for *My computer*, whose lists come live from `COMFY_URL` (the tunnel) and are cached in `_shared/comfy/catalog.json`. Unset → the RunPod dropdowns fall back to that stored catalog, then to the static enum seeds (free text for the installed-file fields). |

...
| `RUNPOD_DATA_CENTER_ID` | **Optional** (e.g. `EU-RO-1`). Scopes the per-card **GPU stock** badge to one region. Stock is reported per GPU *type*, but a stopped pod can only resume where its disk already is, so an unfiltered figure answers a different question. The launcher tries to read each pod's own data centre first and falls back to this; unset, the badge asks globally and says so in its tooltip. The whole fleet shares one region anyway — the Network Volume pins it. |

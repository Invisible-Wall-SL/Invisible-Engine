...
**atlas-backend & atlas-tool:** `COMFY_URL`, `CF_ACCESS_CLIENT_ID`, `CF_ACCESS_CLIENT_SECRET`, `R2_ENDPOINT`, `R2_BUCKET`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, `COMFY_ORG_API_KEY` (optional, gpt_image). **atlas-tool also:** `ATLAS_PROJECT`, `ATLAS_OUTPUT_PREFIX`, `ATLAS_TOOL_SECRET` (optional), `ATLAS_STAGING`, `COMFY_CATALOG_URL` (optional — see below).

> **`COMFY_CATALOG_URL` — where the tools read node CONTRACTS from (`/object_info`: an input's min/max/step, a COMBO's option list).** This feeds the Flipbook/Atlas Maker blueprint importer (a setting's real range and list instead of a guess from the baked value) and the live dropdown refresh in both settings panels. Under `COMFY_TRANSPORT=serverless` the machine that RUNS the graph is a job queue with no HTTP surface, so this is the **only** source consulted there — `COMFY_URL` is deliberately not tried even though it is still set, because that is the local tunnel, a different box whose model folder would fill the dropdowns with the wrong machine's models. **Point it at the always-on CPU volume pod** that launcher-api's `COMFY_VOLUME_POD_ID` names (`https://<podId>-8188.proxy.runpod.net`, no Cloudflare Access in front): it mounts the same Network Volume as the GPU fleet, and every pack the serverless worker installs is a `prod: true` entry of the same `services/atlas-comfy-pod/nodes.json` at the same pinned sha (`ComfyUI-RMBG` included), so the contracts match what production runs. A pod-only pack (`VideoHelperSuite`, `Manager`, …) can describe a class the worker cannot run — that is the blueprint `custom_nodes` check's concern, not this reader's. Read-only by construction: only `/object_info` and `/system_stats` are fetched, so it never starts or bills a pod. **Unset (or the pod asleep) is normal, not an error:** reads answer `ok:false`, the importer falls back to the baked-value guess and says so in the modal, and the panels keep the lists the blueprint was published with.

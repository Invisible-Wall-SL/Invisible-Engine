...
| Var | Service | Purpose |
|---|---|---|
| `COMFY_CATALOG_URL` | **atlas-tool** | **Optional.** Base URL of an always-on ComfyUI to READ installed-model lists from (`/object_info`) for the Settings dropdowns — e.g. the same CPU volume pod the launcher's `COMFY_VOLUME_URL` points at. It is read-only (only `GET /system_stats` + `GET /object_info/<node>`), so it never starts, resumes or bills a pod. Needed because production runs `COMFY_TRANSPORT=serverless`: the RunPod endpoint is a job queue, not an HTTP server, so `COMFY_URL` answers nothing and the ⚙ Settings model fields would otherwise have no live source. Sends the same CF Access headers + `InvisibleAtlas/1.0` UA as every other ComfyUI call. Unset → the tool falls back to the R2 catalog (`_shared/comfy/catalog.json`, refreshed with **⟳ Refresh model lists**) and then to its static enum seeds. |

...
- **atlas-tool / sheet-tool** — there is **no unauthenticated signal at all**. `_gate()` is the first line of both `do_GET` and `do_POST`, so every path including `/` is 403 without `ATLAS_TOOL_SECRET`, and the response carries no commit or deployment header (`x-railway-request-id` is per-request). A 403 proves the service is up and running *our* code; it says nothing about *which commit*. The only proof of BEHAVIOUR is exercising the change through the launcher, signed in — but for the narrower question *"did my commit deploy?"* there is a signal after all, and it needs no Railway token:

...
- **A translation provider on the launcher's Railway service.** Secret, no code default; until one is set the tool loads and edit/save work, but Translate returns a clear "No translation provider configured" error. Either `ANTHROPIC_API_KEY` (a Claude **API** key — a separate paid account from the owner's Claude subscription, which is why the OpenAI-compatible path exists), or `LOCALIZATION_LLM_BASE_URL` + `LOCALIZATION_LLM_API_KEY` (+ `LOCALIZATION_LLM_MODEL` for non-Gemini) to run on a free tier. Also: pick default target languages per project.

## Recent changes
- 2026-08-15 — **provider switch**: `translate.ts` now dispatches to Anthropic *or* any OpenAI-compatible `/chat/completions` endpoint, so translation can run on a free tier (e.g. Google AI Studio) without an Anthropic API account. Verified offline against a stub provider (17 cases: shaping, unrequested-id/lang rejection, URL join, `response_format` retry, fenced JSON, error surfacing, half-configured fallback).

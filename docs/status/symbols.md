...
- **Release the round on arrival** (2026-09-15, default OFF ⇒ byte-parity). Doc-level global
  ...
  only the host section is hidden on a non-emerge project, and stays visible while authored.
  Authored at the foot of the **Transition** section (2026-09-16; it was its own section under
  **Winning symbols explode** on the day it shipped). Full chain: `.strict` Zod + sparse
...
- 2026-09-16 — **The arrival release moved inside the Transition section.** Owner call: it should not
  be a top-level section of its own — it is reached for by someone already looking at the seam
  ...
  **Transition**, in a `wl-config`/`wl-group` block with the section's own `hint` copy — the same
  ...
  - ⚠️ **It is NOT conditional on a transition being bound, and must not become so.** The owner's
    ...
    round (`TumbleBoard.svelte`'s `transitions` doc: "never extends the round"). The wait is the
    emerge ARRIVAL beat, awaited whether or not a transition exists — so an `emerge` project with no
    transition needs this switch exactly as much as one with. Gating it on `doc.transition` would
    strand precisely the projects it was built for. The in-section comment says so at the gate.
  - **UI/IA only.** Same store field `arrivalRelease.enabled`, same `arrivalReleaseEnabled` /
    `setArrivalReleaseEnabled`, same `docSignature` line. The schema, the exporter, the export
    endpoint, the bake whitelist and the engine were not touched, so no runtime release or Borut
    `engine` submodule bump is owed for this change (the 2026-09-15 engine half still owes one).
  - **Availability was WIDENED, never narrowed — the one thing this move could have broken.** The two
    sections were gated differently: the switch was `emerge || arOn`, Transition is
    `emerge || doc.transition`. Folding one into the other as-is would have stranded a non-emerge
    project that had authored the flag but bound no transition — visible sparsely in its doc, with no
    control left to switch it back off. The host gate is therefore
    `data.reelBehaviour.emerge || doc.transition || arOn`. The side effect is deliberate and is the
    point: that project now sees the Transition section too, which is the only place the switch lives.
  - **The copy says what it is, because the placement now implies something false.** The switch does
    not shorten the transition and is not part of it: the transition FX is fire-and-forget and never
    gated the round, whereas the emerge arrival beat is awaited whether or not a transition exists —
    which is also why it is NOT gated on `doc.transition`. An in-section comment records that, so the
    next reader does not "tidy" the gate into `doc.transition &&`.
  - **Verified** `pnpm --filter launcher-api check:clear-reel` (111/111 — it asserts over the client
    setters, the exporter, the endpoint and the bake script, none of which moved) and
    `pnpm --filter launcher-api build`.
...
- 2026-09-15 — **The pop spent its whole budget every paying spin, because every symbol state defaults to LOOP.** Owner report from a live board: after a big win the cells showed no symbols. The pop's beat was never ending on its animation — it was only ever being settled by its cap. Measured on `apps/lines` against the book mock with the switch on: a live track reading `{ anim: 'explosion', duration: 0.533, loop: true }`, restarting ~7.5 times across the 4000 ms budget, with the board being blown apart and re-formed on repeat the whole time before it cleared.
  ...
  - **Verified on this branch**, `apps/lines` + `scripts/mock-rgs-server-book.mjs` with `BIG_WIN=1`, instrumenting raw spine `complete` events against each cell's `symbolState`: the pop fires all 14 winning cells together at `40882ms` with `loop:false, dur 0.53`, and `complete` lands at `41392ms` — **510 ms for the whole pass**, where it previously ran the full 4000 ms budget. **NOT verified:** `pnpm lint` / `svelte-check`. **Engine change — needs a runtime release to reach the online games**, and a Borut `engine` submodule bump for the remake.
...
- 2026-09-15 — **`check:tumble-pattern` was red on every Windows clone, and the trap is the one `check:clear-reel` hit five days earlier.** It reported `1 of 34 FAILED` on any checkout with `core.autocrlf=true` while nothing was wrong: its `read()` handed raw file text to `/\n\t\t\t\ttumblePattern,\n/`, and the whitelist line in `bake-editor-doc.mjs` — present and correct — ends `tumblePattern,\r\n`, so the pattern's TRAILING newline never matched. Exactly 1 of 34 and not more because the sibling assertion on the export endpoint (`/\n\t\t\ttumblePattern,/`) has no trailing `\n` and matched all along: **a LEADING `\n` survives CRLF, a trailing one does not** — which is why this fails in ones and twos rather than obviously, and reads like a real contract breach. Fixed by normalizing in `read()`, copying the helper and its comment verbatim from `check-clear-reel-and-win-explode.ts`, which was given exactly this fix on 2026-09-10 (see that entry below); this file predates it and never picked it up.
  ...
  - **The rule, for the next guard that greps source: `import { readLF } from './lib/read-lf.mjs'`.** Every guard now reads source through that one helper (`scripts/lib/read-lf.mjs`, `readLF(path)` or `lfReaderFrom(baseDir)`) instead of re-typing a `.replace(/\r\n/g, '\n')` each was free to forget — which is what the previous two incidents were. Twenty-four guard files converted. The danger was never the red, it is the repair: a guard that cries wolf gets ignored, or "fixed" by weakening the assertion, which is how a bundle-path check stops checking. Both incidents were found by someone hitting the failure locally, never by CI, because CI runs on LF — so nothing about this class self-reports.
  - **The audit behind that was re-run properly, and the first count was wrong.** Counting per FILE missed unnormalized readers sitting inside files that normalized somewhere else (`verify-reelgrid-perspective-roundtrip.mjs` had both on adjacent lines). Per CALL SITE it was **eight** guards reading source raw, not four: the original `verify-boot-splash`, `verify-clip-reachability`, `verify-scaffold-template`, `verify-win-beat-budget`, plus `verify-reelgrid-perspective-roundtrip`, `verify-symbol-seat`, `check-toolbar-icons` and `verify-launcher-profile`. All eight passed on CRLF by luck of pattern shape; all eight now go through the helper, so the luck is no longer load-bearing. **Three more had to be swept in on 2026-09-18**, having landed on `main` while this sat open — which is the argument for the helper making itself again, three days running: `verify-arrival-release.mts` and `check-symbol-layers.ts` had each re-typed the `.replace(/\r\n/g, '\n')` (green, but the duplication straight back), and `check-big-win-cue.mjs` read source RAW, latent in exactly the way `check:tumble-pattern` was. Deliberately still raw: `check-commit-scope.mjs`, which reads the git commit-message file rather than repo source; `check-path-imports.mjs`, which walks every file in the repo to resolve import specifiers and whose only newline use is a leading `split('\n')` for a line number; `check-stake-consistency.mjs`, which `JSON.parse`s a data file rather than grepping source, so no newline ever reaches an assertion; and the generators (`gen-*`, `sync-*`, `i18n-*`), which read current bytes to compare or rewrite them — normalizing there would rewrite line endings as a side effect.

...
- **Winning symbols explode** (2026-09-10, default OFF ⇒ byte-parity). Doc-global
  `winExplode: { enabled?: boolean }` — once the round has narrated **every** one of its wins, the
  ...
  ONE beat however many cells paid; a capped pop still removes. **Deferred to the END of the round,
  not the end of each win** — a round narrates its wins one after another over the same board and
  ...
  byte-identical to the switch being off. Symbols that did NOT pay are untouched — taking those off
  ...
  `winSymbolCycle.explodeRoundWinners()` → `boardExplodeWinSymbols` → `Board.svelte`, plus a

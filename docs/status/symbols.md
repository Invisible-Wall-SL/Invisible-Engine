...
- **Winning symbols explode** (2026-09-10, default OFF ⇒ byte-parity). Doc-global
  ...
  beat **and is then GONE**: the pop IS the removal (“explode and be gone”), so a symbol goes out
  with a pop on a board that does NOT cascade and nothing pops it a second time. Bounded like the
  win beat (`awaitSymbolBeat` + `WIN_BEAT_CAP_MS`) but with NO floor (`WIN_BEAT_MIN_MS` was already
  spent on the win); a capped pop still removes. Symbols that did NOT pay are untouched — taking
  those off stays the cascade’s / the clear step’s job. The removal is a flag on the reel CELL
  ...
  RESTING replay does not pop AND has nothing left to replay (`winSymbolCycle` marks its passes
  `replay: true`, and `cycleEntries` drops removed cells — so the pop is a choice against the
  replay); a STACKED-COVERED cell is skipped (it mounts no `<Symbol>`, so it could neither draw the
  pop, report it, nor be taken off); and a CASCADE still removes its own winners (its explode step
  skips a seat already gone and sweeps it with the rest). Full chain: `.strict` Zod + sparse

...
- **Winning SYMBOLS keep animating until the next spin** (2026-07-24, default ON). Doc-level
  ...
  winning cells (the deduped union across its wins) via the same `animateSymbols` leaf the
  round used, until the next bet stops it. **Symbols only** — the line and its stamped amount
...
- 2026-07-24 — Winning **symbols** now **keep animating on the resting board until the next spin** (`winSymbolCycle`, authored via the doc-level `winCycle`; default on; the line/amount are NOT replayed) ([detail in history](../history.md)).

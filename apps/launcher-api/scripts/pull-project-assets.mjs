...
// `editor-fonts/`, `editor-symbols/`). Each is ENTIRELY generated by an export
// step (the game never commits into them), so any local file there that the
// current deploy listing no longer carries is a leftover from a previous export
// (e.g. a renamed/removed sheet, a font the project no longer uses, or a symbol
// rebound to a different bundle) and is safe to delete. We scope strictly to
// these subtrees — pruning the rest of `static/assets/` would risk deleting the
// game's committed sheets/fonts (which are NOT in deploy/). `--no-prune` opts out.
const GENERATED_SUBTREES = ['editor-art', 'editor-fonts', 'editor-symbols'];

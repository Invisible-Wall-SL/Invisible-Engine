...
// `editor-fonts/`, `editor-symbols/`, `effects/`). Each is ENTIRELY generated by an
// export step (the game never commits into them), so any local file there that the
...
// rebound to a different bundle, or a deleted Invisible FX effect) and is safe to
// delete. We scope strictly to these subtrees — pruning the rest of `static/assets/`
// would risk deleting the game's committed sheets/fonts (which are NOT in deploy/).
// `--no-prune` opts out.
const GENERATED_SUBTREES = ['editor-art', 'editor-fonts', 'editor-symbols', 'effects'];

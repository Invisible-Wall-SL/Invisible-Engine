...
// `editor-fonts/`). Each is ENTIRELY generated by an export step (the game never
// commits into them), so any local file there that the current deploy listing no
// longer carries is a leftover from a previous export (e.g. a renamed/removed
// sheet, or a font the project no longer uses) and is safe to delete. We scope
// strictly to these subtrees — pruning the rest of `static/assets/` would risk
// deleting the game's committed sheets/fonts (which are NOT in deploy/).
// `--no-prune` opts out.
const GENERATED_SUBTREES = ['editor-art', 'editor-fonts'];
...
async function pruneStaleGenerated() {
	...
	for (const sub of GENERATED_SUBTREES) {
		...
		const keep = new Set(
			files
				.filter((f) => f.rel.startsWith(prefix))
				.map((f) => join(dest, ...f.rel.split('/'))),
		);

...
// 1. The chip compares against THE one kind->protocol map (`mockProtocol.ts`). Its contents are
//    asserted by `scripts/verify-launcher-profile.mts`, which owns that fact; all this needs is
//    that the drift chip reads the same function, and that the shared bundle id is what publish
//    stamps — a chip keyed to a second copy of either would report drift that isn't there.
console.log('1. the shared constants the chips key off:');
check('shared runtime id is still "lines"', SHARED_RUNTIME_ID === 'lines', SHARED_RUNTIME_ID);
check('protocolFor is the real map, not a stub', protocolFor('cluster') === 'cluster');

...
export async function exportSpineBundle(opts: {
	...
	/** Spine load scale; defaults to {@link SPINE_LOAD_SCALE} (= 1, atlas-true). The editor
	 * preview applies the SAME constant so previews are WYSIWYG with the game. */
	scale?: number;
}): Promise<ExportedSpineBundle | null> {
	const { clientKey, projectKey, assetKey, deployPrefix, subtree, stem, skeletonIndex } = opts;
	const scale = opts.scale ?? SPINE_LOAD_SCALE;

<script lang="ts">
	 ...
	 * the param — currently just `action` (clickable). Surfaced only where the def LACKS
	 * the param (a `button` def already declares `action`) — suppressing it there avoids
	 ...
	 * (Per-instance `visibleSource` "Shows during" is retired — Invisible Flow owns
	 * visibility now — so it's no longer offered here.) */
	...
	{:else if node.kind === 'reelGrid'}
		<section>
			...
			<details class="spin-tuning">
				...
				<p class="muted small">
					This section is the board's SHAPE only. Whether a round rolls or <strong
						>swaps symbols in place</strong
					>, which swap style it uses (drop in / column cascade), and the per-column stagger are one
					fact about the GAME rather than about this ratio's layout — a board that rolled in
					portrait and swapped in landscape is not a configuration anyone wants — so they are set
					once in <strong>Game Config → Reel behaviour</strong>.
				</p>
			</details>
		</section>

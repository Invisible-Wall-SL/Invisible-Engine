<script lang="ts">
	 ...
	 * branch uses `anchor={0}` (a spine pivots in its local skeleton frame — see that branch's note);
	 * the mask rectangle then reveals only the top `visibleCells` from the box top.
	 */

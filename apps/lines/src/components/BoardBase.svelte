<script lang="ts">
	 ...
	 * `addChild` + `sortChildren()` on mount), which is the order this list is emitted in — so
	 * back-to-front is bought by ordering ROW-major: back row first, front row last. Deterministic,

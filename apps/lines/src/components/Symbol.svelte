<script lang="ts">
	 ...
	 * therefore had no end for `awaitSymbolBeat` to settle on and burned the full `WIN_BEAT_CAP_MS`,
	 * blowing the symbol apart on repeat for four seconds before the cell reverted to
	 * `postWinStatic`. The default is asked of `props.state` — the state the caller REQUESTED, not
	 * `symbolInfo`'s resolved fallback — because "is this beat terminal?" is a question about the
	 * moment, not about which cell's art ended up drawing it.
	 */
	const loop = $derived(
		props.loop ??

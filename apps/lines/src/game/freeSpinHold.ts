...
import { explodeRoundWinners, startWinCycle, clearWinPresentation } from './winSymbolCycle';
...
export const holdAfterBigWin = async (
	...
	// THE END-OF-ROUND POP (Invisible Symbols → "Winning symbols explode") fires here for the same
	// reason the replay does: this is the OTHER point at which a spin's presentation has finished and
	// the board is handed back to the player. Awaited before the replay starts, exactly as in
	// `playBet`'s `finally` — the two seams run the same pair in the same order, so a held spin and
	// an ordinary one cannot end differently. A no-op with the switch off.
